...
AJASourceMultiChannelOp::AJASourceMultiChannelOp() {
  HOLOSCAN_LOG_INFO("AJASourceMultiChannelOp::AJASourceMultiChannelOp - Constructor called");
}

void AJASourceMultiChannelOp::setup(OperatorSpec& spec) {
...
AJAStatus AJASourceMultiChannelOp::DetermineVideoFormat() {
...
AJAStatus AJASourceMultiChannelOp::OpenDevice() {
...
AJAStatus AJASourceMultiChannelOp::SetupVideo() {
  ...
  if (enable_overlay_) {
    ...
    for (size_t i = 0; i < num_overlays; ++i) {
      ...
      device_.SetReference(NTV2_REFERENCE_INPUT2); //TODO we need to figure out which one to use here
...
bool AJASourceMultiChannelOp::AllocateBuffers(std::vector<void*>& buffers, size_t num_buffers,
...
void AJASourceMultiChannelOp::FreeBuffers(std::vector<void*>& buffers, bool rdma) {
  for (auto& buf : buffers) {
    if (rdma) {
      if (is_igpu_) {
        cudaFreeHost(buf);
...
AJAStatus AJASourceMultiChannelOp::SetupBuffers() {
...
void AJASourceMultiChannelOp::initialize() {
  ...
  // spec()->outputs()["video_buffer_output"]->condition(ConditionType::kNone);
...
void AJASourceMultiChannelOp::start() {
...
void AJASourceMultiChannelOp::compute(InputContext& op_input, OutputContext& op_output,
                          ExecutionContext& context) {
  HOLOSCAN_LOG_TRACE("=== AJA Source Compute Start ===");
  HOLOSCAN_LOG_TRACE("Double buffering state - Channel buffers: [{}]", 
    ...
    for (size_t i = 0; i < num_overlay_channels; ++i) {
      ...
      if (!maybe_overlay_message || maybe_overlay_message.value().is_null()) {
        ...
        try {
          ...
          HOLOSCAN_LOG_TRACE("Received overlay input for channel {}", i + 1);
    ...
    if (std::any_of(have_overlay_in.begin(), have_overlay_in.end(), [](bool b) { return b; })) {
      ...
      for (size_t i = 0; i < num_overlay_channels; ++i) {
        if (have_overlay_in[i] && overlay_buffers[i]) {
          ...
          HOLOSCAN_LOG_TRACE("Applied overlay to channel {} using hardware frame {}", i + 1, hw_overlay_frame);
        }
      }
  ...
  for (size_t i = 0; i < num_channels; ++i) {
    ...
    HOLOSCAN_LOG_TRACE("=== CHANNEL {} BUFFER MAPPING ===", i + 1);
    HOLOSCAN_LOG_TRACE("  Channel: NTV2_CHANNEL{}", static_cast<int>(channel));
    HOLOSCAN_LOG_TRACE("  Hardware Frame: {}", hw_frame);
    HOLOSCAN_LOG_TRACE("  Allocated Buffer Index: {}", buffer_index);
    HOLOSCAN_LOG_TRACE("  Current Buffer: {}", static_cast<int>(current_buffer));
    HOLOSCAN_LOG_TRACE("  Buffer Address: 0x{:x}", reinterpret_cast<uintptr_t>(ptr));
    ...
    HOLOSCAN_LOG_TRACE("  Successfully read from hardware frame {} to buffer {}[{}] (NTV2_CHANNEL{})", 
  ...
  if (enable_overlay_) {
    for (size_t i = 0; i < num_overlay_channels; ++i) {
      ...
      HOLOSCAN_LOG_TRACE("Read overlay data from hardware frame {} to buffer {}[{}] for channel {}", 
      ...
      HOLOSCAN_LOG_TRACE("Successfully emitted overlay output for channel {} from buffer {}[{}]", 
  ...
  for (size_t i = 0; i < num_channels; ++i) {
    ...
    op_output.emit(result, output_name);
    HOLOSCAN_LOG_TRACE("Successfully emitted video output for channel {} (NTV2_CHANNEL{}) from buffer {}[{}]", 
                      i + 1, static_cast<int>(channel), channel_to_buffer_map_[channel], static_cast<int>(filled_buffer_indices[i]));
  }

  HOLOSCAN_LOG_TRACE("=== AJA Source Compute Complete ===");
  HOLOSCAN_LOG_TRACE("Buffer flow: Each channel alternates between buffer 0 and 1 for smooth capture");
  HOLOSCAN_LOG_TRACE("Next tick will use buffers: [{}]", fmt::join(current_channel_buffers_, ", "));
}

void AJASourceMultiChannelOp::stop() {
...
bool AJASourceMultiChannelOp::GetNTV2VideoFormatTSI(NTV2VideoFormat* format) {
  switch (*format) {
